...
export const getUserById = async (id) => {
  try {
    const response = await axios.get(
      `https://localhost:7169/api/Users/${id}`,
      authHeader()
    );
...
export const getStaff = () => {
  return axios
    .get("https://localhost:7169/api/Users/GetStaffs",getAuthHeader())
    .then((res) => res.data);
};
...
export const updateUser = (userId, data) => {
  ...
  return axios.post(`https://localhost:7169/api/Users/PutUser`, data, {
    headers: {
      Authorization: `Bearer ${token}`,
    },
  });
...
export const addUser = (data) => {
  return axios
    .post("https://localhost:7169/api/Auth/register", data, authHeader())
    .then((res) => res.data);
};
